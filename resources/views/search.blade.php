        ...
        <div class="container">
            <div class="row justify-content-center align-items-center d-flex">
                <div class="col-lg-8">
                        ...
                        <form action="{{route('search')}}" method="GET">
                            <div class="input-group stylish-input-group">
                                <input type="text" class="form-control" placeholder="Addictionwhen gambling"
                                       ...
                                       onblur="this.placeholder = 'Addictionwhen gambling '" required="" name="search"
                                       value="{{$search ?? ""}}">
                    ...
                    <p class="mt-20 text-center text-white">{{$posts->count() ?? "0"}} results found for
                        “{{$search ?? ""}}”</p>
                </div>
            </div>
        </div>
                                        ...
                                        @foreach ($posts as $post)
                                            <div class="single-posts col-lg-6 col-sm-6">
                                                <img class="img-fluid" src="{{asset('storage/post/'.$post->image)}}"
                                                     alt="{{$post->image}}"/>
                                                <div
                                                    class="date mt-20 mb-20">{{$post->created_at->diffForHumans()}}</div>
                                                <div class="detail">
